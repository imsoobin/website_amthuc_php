<?php
    ...
    if(isset($_POST['sapxepgia'])){
        ...
        {
            $sql = "SELECT * FROM mon_an ORDER BY gia ASC ";
            $query_sapxep = mysqli_query($conn, $sql);
        }
        else
        {
            $sql = "SELECT * FROM mon_an ORDER BY gia DESC ";
            $query_sapxep = mysqli_query($conn, $sql);
        }
